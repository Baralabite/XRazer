...
public class XRazerCommandExecutor{
	...
	/*
	 * Name: onCommand
	 * Description: Gets called when a command is executed
	 * Returns: boolean
	 * Parameters: CommandSender, Command, String, String[]
	 * Requirements: None
	 */
	...
	public void commandWhitelistList(Player player, String[] args){
		List<String> whitelistedplayers = parent.getConfig().getStringList("WhitelistedPlayers");
		player.sendMessage(ChatColor.BLUE+"=====["+ChatColor.GOLD+"Whitelisted Players"+ChatColor.BLUE+"]=====");					
	...
	public void commandWhitelistAdd(Player player, String[] args){
		...
		if (!isWhitelisted){
			...
			parent.saveConfig();
			player.sendMessage(ChatColor.RED+args[2]+ChatColor.AQUA+" added to the whitelist.");
		}
		else{
			player.sendMessage(ChatColor.RED+"ERROR: Player already whitelisted.");
		}
	}
	...
	public void commandWhitelistRemove(Player player, String[] args){
		...
		if (isWhitelisted){
			...
			parent.saveConfig();
			player.sendMessage(ChatColor.RED+args[2]+ChatColor.AQUA+" removed from the whitelist.");
		}
		else{
	...
	public void commandCheck(Player player, String[] args){
		...
		try{
			...
			totalScore = parent.blocklogger.getPlayerScore(target).getTotalScore();
		} catch(NullPointerException ex){
			player.sendMessage(ChatColor.RED+player.getName()+ChatColor.AQUA+" XRaying Statistics");
			player.sendMessage(ChatColor.DARK_AQUA+"=======================================");	
			player.sendMessage(ChatColor.BLUE+"Total: "+ChatColor.GREEN+"0");
			return;
		}
	    ...
	    while (it.hasNext()) {
	        ...
	        player.sendMessage(ChatColor.GRAY+(String) pairs.getKey()+": "+String.valueOf(pairs.getValue()));	        
	    }
	    String total;
			...
			if (args[0].equalsIgnoreCase("check")){
				if (player.hasPermission("xrazer.check")){
					if (!(args.length < 2)){
						...
						HashMap<String, Integer> ratios;
						int stonescore;
						float ratio;
						
						try{
							stonescore = parent.blocklogger.ratios.get(parent.getServer().getPlayer(args[1])).getStoneScore();						
						}
						catch (Exception ex){
							stonescore = 0;
						}
						try{
							ratios = parent.blocklogger.ratios.get(parent.getServer().getPlayer(args[1])).ratio;
							ratios.remove("Stone");
						}
						catch (Exception ex){
						...
						try{
							ratio = parent.blocklogger.ratios.get(parent.getServer().getPlayer(args[1])).getRatio();							
						}
						catch (Exception ex){
							ratio = 0.0F;
							System.out.println("C");
						}
						player.sendMessage(ChatColor.RED+args[1]+ChatColor.AQUA+" XRaying Statistics.");
						player.sendMessage(ChatColor.DARK_AQUA+"========================================");										
						player.sendMessage(ChatColor.GRAY+"Stone Mined: "+ChatColor.GREEN+stonescore);					
						Iterator it = ratios.entrySet().iterator();
					    while (it.hasNext()) {
					        Map.Entry pairs = (Map.Entry)it.next();
					        player.sendMessage(ChatColor.BLUE+(String) pairs.getKey()+": "+ChatColor.GREEN+pairs.getValue());
					        it.remove();
					    }
				...
				if (!(args.length < 2)){				
					if (args[1].equalsIgnoreCase("add")){
						if (player.hasPermission("xrazer.whitelist")){
							List<String> whitelistedplayers = parent.getConfig().getStringList("WhitelistedPlayers");
							if (whitelistedplayers.contains(args[2])==true){
								player.sendMessage(ChatColor.RED+"ERROR: That player is already on the whitelist configuration.");
								return true;
							}
							else{
						...
						if (player.hasPermission("xrazer.whitelist")){
								List<String> whitelistedplayers = parent.getConfig().getStringList("WhitelistedPlayers");
							if (whitelistedplayers.contains(args[2])==false){
								player.sendMessage(ChatColor.RED+"ERROR: That player is not whitelisted in the configuration.");
								return true;
							}
							else{
				...
				if (args[1].equalsIgnoreCase("list")){
					if (player.hasPermission("xrazer.whitelist")){
					List<String> whitelistedplayers = parent.getConfig().getStringList("WhitelistedPlayers");
					player.sendMessage(ChatColor.BLUE+"=====["+ChatColor.GOLD+"Whitelisted Players"+"]=====");					
					for (String ply: whitelistedplayers){
						player.sendMessage(ChatColor.GREEN+ply);
					}
					player.sendMessage(ChatColor.BLUE+"===============================");
